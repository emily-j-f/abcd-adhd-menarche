# abcd-adhd-menarche
These are all the R code files for a study investigating how menarche might be associated with changes in ADHD traits in the general adolescent population and whether it moderates mental health outcomes in adolescents with ADHD vs those without. Comparative analyses were also performed to see if effects differ between menarche and breast development. Data from the Adolescent Brain and Cognitive Development (ABCD) study release 6.0 were used for all analyses.

Follow these steps to repeat the study:

1. [Data processing](https://github.com/emily-j-f/abcd-adhd-menarche/tree/release-6.0/1.%20ABCD%20data%20processing)
2. Generate [descriptive statistics](https://github.com/emily-j-f/abcd-adhd-menarche/tree/release-6.0/2.%20Descriptives)
3. Perform the [main analyses](https://github.com/emily-j-f/abcd-adhd-menarche/tree/release-6.0/3.%20Analyses%20(Bayesian%20GLMM)) (Bayesian generalised linear mixed models)
4. Perform [sensitivity analyses](https://github.com/emily-j-f/abcd-adhd-menarche/tree/release-6.0/4.%20Supplementary%3Asensitivity%20analyses) to test the robustness of effects.
  
A codebook for all variables can be found [here](https://github.com/emily-j-f/abcd-adhd-menarche/tree/release-6.0/Codebook).

PDF files of R outputs for main analyses and supplementary/sensitivity analyses can be found in their respective folders.

Final checks of code and any necessary fixes for the version 1.1.0 release were performed the week of 3 August 2026. Main findings/conclusions were unaffected. 

DOI for this repository: [https://doi.org/10.5281/zenodo.21348585](https://doi.org/10.5281/zenodo.21348585)

